...
OPTIONAL{?p16 rdf:type omop:production_id.
    ?instance_owl ?p16 ?production_id.}
...
OPTIONAL{?p17 rdf:type omop:device_unit_concept_id.
    ?instance_owl ?p17 ?unit_concept_id_uri.
    ?unit_concept_id_uri owl:sameAs ?omop_p17.
    ?omop_p17 std:domain_id ?omop_domain_p17.
      FILTER(?omop_domain_p17="Unit" || ?omop_p17 = <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0>)
      ?omop_p17 std:concept_id ?device_unit_concept_id.}   
OPTIONAL{?p18 rdf:type omop:device_unit_source_value.
    ?instance_owl ?p18 ?device_unit_source_value_uri.
    ?device_unit_source_value_uri ?p14bis ?unit_source_value.}
OPTIONAL{?p19 rdf:type omop:device_unit_source_concept_id.
    ?instance_owl ?p19 ?device_unit_source_concept_id_uri.
    ?device_unit_source_concept_id_uri owl:sameAs ?omop_p19.
    ?omop_p19 std:domain_id ?omop_domain_p19.
      FILTER(?omop_domain_p19="Unit" || ?omop_p19 = <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0>)
      ?omop_p19 std:concept_id ?unit_source_concept_id.}
}
"""
    return query
