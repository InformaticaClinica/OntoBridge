...
OPTIONAL{?p21 rdf:type omop:unit_source_concept_id.
    ?instance_owl ?pp21 ?unit_source_concept_id_uri.
    ?unit_source_concept_id_uri owl:sameAs ?omop_pp21.
    ?omop_p21 std:domain_id ?omop_domain_p21.
      FILTER(?omop_domain_p21="Unit" || ?omop_p21 = <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0>)
      ?omop_p21 std:concept_id ?unit_source_concept_id.}
...
OPTIONAL{?p22 rdf:type omop:measurement_event_id.
    ?instance_owl ?p22 ?measurement_event_id.}
OPTIONAL{?p23 rdf:type omop:meas_event_field_concept_id.
    ?instance_owl ?p23 ?meas_event_field_concept_id.}
 }
"""
    return query
