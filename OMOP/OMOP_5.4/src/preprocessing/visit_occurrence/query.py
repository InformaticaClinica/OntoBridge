def query_visit_occurrence():
    ...
    SELECT ?visit_occurrence_id ?person_id ?visit_concept_id ?visit_start_date ?visit_start_datetime ?visit_end_date ?visit_end_datetime ?visit_type_concept_id ?provider_id ?care_site_id ?visit_source_value ?visit_source_concept_id ?admission_source_concept_id ?admitted_from_source_value ?discharge_to_concept_id ?discharge_to_source_value ?preceding_visit_occurrence_id
WHERE {
...
?p11 rdf:type omop:visit_source_value.
    ?instance_owl ?p11 ?visit_source_value_1.
OPTIONAL{?visit_source_value_1 owl:sameAs ?omop_p11.
    ?omop_p11 std:domain_id ?omop_domain_p11.
      FILTER(?omop_domain_p11="Visit" || ?omop_p11= <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0> )
      ?omop_p11 std:concept_code ?visit_source_value.}
?p12 rdf:type omop:visit_source_concept_id.
    ?instance_owl ?p12 ?visit_source_concept_id_1.
OPTIONAL{?visit_source_concept_id_1 owl:sameAs ?omop_p12.
...
OPTIONAL{?p13 rdf:type omop:visit_admission_source_concept_id.
    ?instance_owl ?p13 ?admission_source_concept_id_1.
    ?admission_source_concept_id_1 owl:sameAs ?omop_p13.
    ?omop_p13 std:domain_id ?omop_domain_p13.
      ...
      ?omop_p13 std:standard_concept_id ?admission_source_concept_id_uri.
      ?admission_source_concept_id_uri std:concept_id ?admission_source_concept_id.}
OPTIONAL{?p14 rdf:type omop:visit_admitted_from_source_value.
