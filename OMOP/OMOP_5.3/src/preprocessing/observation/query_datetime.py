def query_observation_datetime():
    query = """
...
?p6 rdf:type omop:observation_type_concept_id.
...
?p15 rdf:type omop:observation_source_value.
    ?instance_owl ?p15 ?observation_source_value_1.
OPTIONAL{?observation_source_value_1 owl:sameAs ?omop_p15.
    ?omop_p15 std:domain_id ?omop_domain_p15.
      FILTER(?omop_domain_p15="Observation" || ?omop_p15= <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0> )
      ?omop_p15 std:concept_code ?observation_source_value.}
?p16 rdf:type omop:observation_source_concept_id.
    ?instance_owl ?p16 ?observation_source_concept_id_1.
OPTIONAL{?observation_source_concept_id_1 owl:sameAs ?omop_p16.
...
OPTIONAL{?p17 rdf:type omop:observation_unit_source_value.
    ?instance_owl ?p17 ?unit_source_value.}
OPTIONAL{?p18 rdf:type omop:qualifier_source_value. 
    ?instance_owl ?p18 ?qualifier_source_value.}
OPTIONAL{?p19 rdf:type omop:observation_event_id.
    ?instance_owl ?p19 ?observation_event_id.}
OPTIONAL{?p20 rdf:type omop:obs_event_field_concept_id.
    ?instance_owl ?p20 ?obs_event_field_concept_id_1.
    ?obs_event_field_concept_id_1 owl:sameAs ?omop_p20.
    ?omop_p20 std:domain_id ?omop_domain_p20.
    FILTER(?omop_domain_p20="Observation" || ?omop_p20= <http://ontoar.clinic.cat/ontologias/standard_concepts.owl#OMOP_0> )
    ?omop_p20 std:standard_concept_code ?obs_event_field_concept_id_uri.
    ?obs_event_field_concept_id_uri std:concept_id ?obs_event_field_concept_id.}
?p21 rdf:type omop:value_as_datetime.
    ?instance_owl ?p21 ?value_as_datetime.
}"""
    return query
