...
def post_omop(OMOP_therm, read_folder, write_folder, all):
    ...
    if '_datetime' in container_df.columns:
        container_df['_datetime'] = container_df['_datetime'].str.split(' ').str[1].fillna("00:00:00")

    if "_date" in container_df.columns:
        container_df["_date"] = container_df["_date"].str.split(' ').str[0].fillna(container_df["_date"])
